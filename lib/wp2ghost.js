...
var convertHtmlToMarkdown = function(html, title) {
  ...
  // 3. Convert [audio], [video] and [youtube] boxes
  ...
  // Example: [youtube https://www.youtube.com/watch?v=tSFdLSB1nAA]
  html = html.replace(/[[]youtube[= ]?(http.+?)]/g, "$1");
  ...
  // 5. Some sanity checks / assertions (?)
  ...
  // 7. Embed youtube links using HTML
  var yt_counter = 0;
  function embedYoutubeVideo(match, yt_id) {
    yt_counter += 1;
    return '<iframe width="560" height="315" src="https://www.youtube.com/embed/' + yt_id + '" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
  }
  var u = require('util');
  //if (title.includes('Devcon IV'))
  //  console.log(u.inspect(md));
  var yt_occurrences = countAllOccurrences(md, "youtube") + countAllOccurrences(md, "youtu.be");
  md = md.replace(/^http(?:s?):\/\/(?:www\.)?youtu(?:be\.com\/watch\?v=|\.be\/)([\w\-\_]*)(&(amp;)?[\w\?=]*)*$/mg, embedYoutubeVideo);
  if (yt_occurrences != yt_counter) {
    console.log("[WARN] Post " + title + " youtube occurences vs regex hits: " + yt_occurrences + ":" + yt_counter);
  }

  // 8. Embed twitter links
  // TODO

  // 9. Bring back code listings (this needs to be after embedding yt/twitter so they don't modify code listings!)
...
var countAllOccurrences = function(string, search) {
  var startIdx = 0;
  var occurences = 0;
  var index;
  while ((index = string.indexOf(search, startIdx)) > -1) {
    occurences += 1;
    startIdx = index + search.length;
  }
  return occurences;
}
    ...
    xml.on('endElement: item', function(item) {
      ...
      if (!post.slug) {
        if (!GENERATE_SLUG_IF_NOT_PRESENT) {
          console.log("[INFO] post " + item['link'] + " ignored as it has no slug");
          return;
        }
        post.slug = slugify(post.title);
        console.log("[INFO] post " + item['link'] + " has no slug; generated one: " + post.slug);
      }
